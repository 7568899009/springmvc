...
<link rel="stylesheet"
	...
	<script type="module">
		$('.delete').on('click', function () {
			if (confirm("您是否要删除?")) {
			location.href="${pageContext.request.contextPath}/user/delete/"+$(this).parents("td").siblings().eq(1).text();
			}
		});
</script>
						...
						<c:forEach items="${userList}" var="users">
										<tr>
											...
											<td class="text-center">
												<c:forEach items="${users.roles}" var="item">
													「${item.roleName}」
												</c:forEach>
											</td>
											<td class="text-center">
												<a href="javascript:void(0);" class="btn delete bg-olive btn-xs">删除</a>
											</td>
										</tr>
						</c:forEach>
